...
function updateAdminUI() {
    ...
    if (title) {
        if (currentUserRole === 'head') {
            title.innerText = `👑 ตรวจสอบรายการ (แผนก${currentUserDept})`;
        } else if (currentUserRole === 'manager') {
            title.innerText = '👑 รายการรออนุมัติ (คุณเบญจมาศ)';
        }
    }
}
...
if (prForm) {
    prForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (!headEmail) {
                alert("⚠️ ไม่พบอีเมลหัวหน้าของแผนกนี้ กรุณาติดต่อ Admin");
            ...
            await emailjs.send(CONFIG.emailServiceId, CONFIG.emailTemplateId_Master, { 
                ...
                html_content: `<h3>เรียน หัวหน้าแผนก${dept},</h3><p>มีรายการขอซื้อใหม่จาก <b>${payload.requester}</b> รอการตรวจสอบครับ</p><p>เลขที่ PR: ${payload.pr_number}</p><p><a href="${adminLink}">คลิกเพื่อเข้าสู่ระบบ (ใช้รหัสของแผนกท่าน)</a></p>` 
            });
...
async function loadPRs() {
    ...
    try {
        ...
        if (currentMode === 'pending') {
            ...
            if (currentUserRole === 'head') {
                query = query.neq('status', 'pending_head').eq('department', currentUserDept);
            } else {
